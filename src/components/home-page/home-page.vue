          ...
          <div class="model" :class="model">
            <ul style="padding: 0">
              <li v-for="story in stories" :key="story.id" class="new border-1px" @click="goNew(story.id)" :class="model">
                ...
                <span class="avatar" v-for="(item,index) in story.images" v-if="index<1"><img :src="attachImageUrl(item)"></span>
              </li>
            </ul>
          </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        fetchMoreDate() {
            ...
            axios
                .get("api/news/before/" + this.homepageDateStr)
                .then(response => {
                    let stories = response.data.stories;
                    let ids = stories.map(story => story.id);
        ...
        ...mapActions(["addNews", "addDate", "addDateStr", "setGoType"])
    },
    computed: {
        model() {
